...
class TransactionBuilder
{
    ...
    public function createToken($options = [], $issuerAddress = null)
    {
        ...
        if (
            !is_integer($options['freeBandwidthLimit']) || $options['freeBandwidthLimit '] < 0 ||
        ...
        if (
            !is_integer($options['frozenAmount']) || $options['frozenAmount '] < 0 ||
        ...
        if (
            !is_integer($options['frozenDuration']) || $options['frozenDuration '] < 0 ||
            ($options['frozenDuration'] && !$options['frozenAmount'])
        ) {
